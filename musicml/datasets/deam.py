...
class DEAMDataset:
    ...
    with discretized arousal and valence labels.

    Supports two modes:
    - On-the-fly: loads audio and computes features per __getitem__
    - Cached: loads pre-computed .npz files from feature_cache_dir
    """
    ...
    def _build_samples(self, df: pd.DataFrame) -> list[dict[str, Any]]:
        """Build list of (track_id, window_start_sec, arousal, valence) samples."""
